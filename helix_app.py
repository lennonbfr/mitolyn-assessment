...
def log_event(event, detail=""):
    print(f"[TRACKING] {event.upper()} | {detail}")
...
def generate_pdf(name, age, score, status, description, metabolic_age, answers):
    ...
    start_y = 530
    c.drawString(100, start_y, "3. Mitochondrial Pattern Analysis")

    p = Paragraph(description, body_style)

    width, height = p.wrap(400, 200)

    p.drawOn(c, 120, start_y - height - 10)

    # INDICATORS
    next_y = start_y - height - 50
...
def main():
    ...
    # STEP 2
        ...
        q1 = st.radio(
            "1. How do you feel 30 minutes after waking up?",
            ["Energized", "A little slow", "Tired and unmotivated"],
            index=None
        )

        q2 = st.radio(
            "2. Do you experience an afternoon energy crash?",
            ["Rarely", "Sometimes", "Almost every day"],
            index=None
        )

        q3 = st.radio(
            "3. Do you feel colder than other people?",
            ["No", "Sometimes", "Yes, often"],
            index=None
        )

        q4 = st.radio(
            "4. Do you struggle to lose weight even eating less?",
        ...
        if st.button("Generate My Report"):
            ...
            if all([q1, q2, q3, q4, q5]):
                ...
                total_pts = sum(
                    mapping.get(q, 1) for q in [q1, q2, q3, q4, q5]
                )
                ...
                if score <= 40:
                    ...
                    desc = (
                        "Your results suggest a pronounced reduction in cellular energy efficiency. "
                        "This pattern is often associated with slower metabolic activity and reduced fat-burning responsiveness."
                    )
                    ...
                    desc = (
                        ...
                        "Your metabolism may currently be working below its ideal cellular potential."
                    )
                    ...
                    desc = (
                        "Your cellular energy profile appears relatively stable, although there may still be room "
                        "to improve metabolic efficiency and long-term energy balance."
                    )
                ...
                log_event("quiz_completed", f"score={score}")
    ...
    # STEP 3
        ...
        with st.status("Scanning mitochondrial markers...", expanded=True) as s:
            ...
            s.update(
                label="Matching age-related energy patterns...",
                state="running"
            )
    ...
    # STEP 4
        ...
        st.warning(
            "Your results suggest that reduced cellular energy may be affecting how efficiently your body burns fuel."
        )
        ...
        with st.form("lead_form"):
            ...
            submitted = st.form_submit_button("UNLOCK REPORT")
